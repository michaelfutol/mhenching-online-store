...
The online store may use a separate Supabase project later. Until a project is explicitly selected/created, schema files in this repo are **design drafts**, not applied migrations.

Current Supabase guidance matters here: tables in exposed schemas require explicit API exposure/grants in newer projects, and exposed tables must use RLS. Sensitive commerce tables should therefore live in a non-exposed schema where possible, with server-only access and additional RLS/permissions as defense in depth.
